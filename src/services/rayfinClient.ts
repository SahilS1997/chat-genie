...
import type { ChatGenieFunctions } from '../../rayfin/functions/schema';
...
let client: RayfinClient<BlankAppSchema, ChatGenieFunctions> | null = null;
let localDev = false;

export async function initRayfinClient(
  config: RayfinClientConfig
): Promise<RayfinClient<BlankAppSchema, ChatGenieFunctions>> {
  if (client) {
  ...
  client = new RayfinClient<BlankAppSchema, ChatGenieFunctions>({
...
export function getRayfinClient(): RayfinClient<BlankAppSchema, ChatGenieFunctions> {
  if (!client) {
    throw new Error(
